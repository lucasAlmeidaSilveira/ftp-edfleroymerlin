    ...
    <section class="js-adbar section-adbar px-1">
        <div class="js-swiper-adbar swiper-container text-center">
            <div class="swiper-wrapper align-items-center">
                {% set counter = 0 %} {# Inicializa um contador #}
                {% for adbar in ['ad_bar_01', 'ad_bar_02', 'ad_bar_03'] %}
                    {% set counter = counter + 1 %} {# Incrementa o contador #}
                    {% set advertising_text = attribute(settings,"#{adbar}_text") %}
                    {% set advertising_url = attribute(settings,"#{adbar}_url") %}
                    {% if advertising_text %}
                        <div class="swiper-slide slide-container px-4 {% if counter == 2 %}subscribeButton{% endif %}" {% if counter == 2 %}style="cursor: pointer;"{% endif %}>
                            {% if advertising_url %}
