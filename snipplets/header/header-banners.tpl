                    ...
                    {% if head_informative_banner_icon != 'none' %}
                        <div class="col-auto pr-0">
                            ...
                            {% elseif head_informative_banner_icon == 'shipping' %}
                                {% include "snipplets/svg/package.tpl" with {svg_custom_class: "icon-inline utilities-icon"} %}
                            {% elseif head_informative_banner_icon == 'store' %}
                                {% include "snipplets/svg/store.tpl" with {svg_custom_class: "icon-inline utilities-icon"} %}
                            {% elseif head_informative_banner_icon == 'card' %}
                                {% include "snipplets/svg/credit-card-alt.tpl" with {svg_custom_class: "icon-inline utilities-icon"} %}
                            {% elseif head_informative_banner_icon == 'cash' %}
                                {% include "snipplets/svg/cash-alt.tpl" with {svg_custom_class: "icon-inline utilities-icon"} %}
                            {% endif %}
                        </div>
                    {% endif %}
                    {% if head_informative_banner_title or header_banner_has_link %}
                        <div class="{% if has_1_banner %}col-auto{% else %}col{% endif %} {% if has_1_banner or head_informative_banner_icon == 'none' %} justify-content-center{% endif %} {% if head_informative_banner_icon == 'none' %}col-md-auto text-center text-md-left{% endif %} pl-2 ml-1">
                    {% endif %}
                        {% if head_informative_banner_title %}
                            <div class="font-smallest">{{ head_informative_banner_title }}</div>
                        {% endif %}
                        {% if header_banner_has_link %}
                            <div class="btn-link">{{ head_informative_banner_link_text }}</div>
                        {% endif %}
                    {% if head_informative_banner_title or header_banner_has_link %}
                        </div>
                    {% endif %}
