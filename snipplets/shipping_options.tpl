{% if options %}
    ...
    <div class="{% if cart.items_count > 0 and not cart.free_shipping.cart_has_free_shipping %}js-product-shipping-label{% endif %} font-small mb-4 {% if not store.show_shipping_emergency_message %}mt-2{% endif %}" style="display: none;">
    ...
    {% if has_featured_shipping %}
        ...
        <div class="font-smallest mb-3">
            {{ "Envío a domicilio" | translate }}
        </div>

        
        <ul class="radio-button-container list-unstyled mb-2">
    ...
    {% if store.has_smart_dates and show_time %}
        <div class="font-small pb-2">{{"El tiempo de entrega <strong>no considera feriados</strong>." | translate}}</div>
    {% endif %}
{% else %}
