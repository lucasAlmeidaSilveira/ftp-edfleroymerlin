		...
		<div class="row">
			<div class="col-md-5">
				...
				{% else %}
					...
					{% endembed %}
				{% endif %}
				<p class="mt-3 {% if account_validation == 'pending' %}text-left{% else %}text-center{% endif %}">{{ '¿Ya tenés una cuenta?' | translate }} {{ "Iniciá sesión" | translate | a_tag(store.customer_login_url, '', 'btn-link') }}</p>
			</div>
		</div>
	</div>
</section>
